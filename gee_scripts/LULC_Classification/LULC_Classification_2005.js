...
// ====================================
// 2) LULC TRAINING POINTS (use/adjust coordinates)
// ====================================
...
// =========================================
// 3) LANDSAT 5 SR IMAGE CLIP (2005)
// =========================================
var bands05 = ['SR_B3','SR_B4','SR_B5'];
var image05_full = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2')
  .filterBounds(aoi_simple)
  .filterDate('2005-03-01', '2005-05-31')
  .filter(ee.Filter.lt('CLOUD_COVER', 20))
  .median();
var image05 = image05_full.select(bands05);

Map.addLayer(image05_full.clip(aoi_simple), {bands: ['SR_B4', 'SR_B3', 'SR_B5'], min: 0, max: 3000}, 'Landsat 5 RGB 2005 (clipped)');
...
var training05 = image05.sampleRegions({
...
// 2. Train RF
var classifier05 = ee.Classifier.smileRandomForest(50).train({
  features: training05,
  classProperty: 'class',
  inputProperties: bands05
});

// 3. Classify AOI
var lulc05 = image05.clip(aoi_simple).classify(classifier05);

Map.addLayer(lulc05, {
  min: 0, max: 5,
  palette: ['228B22', 'FFD700', '808080', '1E90FF', 'D2B48C', '7CFC00']
}, 'LULC Classified 2005 (RF)');

// 4. Export classified map to Drive
Export.image.toDrive({
  image: lulc05,
  description: 'LULC_RF_classified_2005',
  scale: 30,
  region: aoi_simple,
  fileFormat: 'GeoTIFF',
  maxPixels: 1e9

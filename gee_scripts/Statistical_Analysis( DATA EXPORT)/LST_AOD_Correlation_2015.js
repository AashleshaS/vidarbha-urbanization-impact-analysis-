...
var lst2015 = landsat2015.map(calcLST8).select('LST').median().clip(aoi_simple);

// =========================
// PM2.5 Proxy (AOD, MODIS) 2015
// =========================
var aod2015 = ee.ImageCollection('MODIS/061/MYD08_M3')
  .filterBounds(aoi_simple)
  .filterDate('2015-03-01', '2015-05-31')
...
// Sampling and Export
// =========================
var points2015 = ee.FeatureCollection.randomPoints({
  region: aoi_simple,
  points: 500,
  seed: 2015
});
var stacked2015 = lst2015.addBands(aod2015);
var samples2015 = stacked2015.sampleRegions({
  collection: points2015,
  scale: 1000,
  geometries: true
});
Export.table.toDrive({
  collection: samples2015,
  description: 'Vidarbha_Sample_LST_AOD_2015',
  fileFormat: 'CSV'
});
